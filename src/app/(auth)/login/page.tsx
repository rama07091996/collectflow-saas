...
export default function LoginPage() {
  ...
  const handleVerify2FA = (e: React.FormEvent) => {
    ...
    const isValid = JWTAuthService.verifyTwoFactorCode(
      twoFactorCode,
    ...
    <div className="min-h-screen bg-slate-900 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center relative z-10">
        ...
        <h2 className="text-2xl font-bold text-white tracking-tight">
          {is2FAPrompt ? 'Two-Factor Authentication' : 'Sign in to your account'}
        </h2>
        <p className="mt-1 text-xs text-slate-400">
          {is2FAPrompt
            ? 'Enhanced enterprise security protection is enabled'
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        <div className="bg-slate-950/80 backdrop-blur-xl py-8 px-6 shadow-2xl border border-slate-800 sm:rounded-2xl sm:px-10 space-y-6">
          {/* ------------------------------------------------------------- */}
          {/* STEP 2: TWO-FACTOR MULTI-FACTOR AUTHENTICATION SCREEN         */}
          {/* ------------------------------------------------------------- */}
          {is2FAPrompt ? (
            <div className="space-y-5 animate-in fade-in zoom-in-95 duration-200">
              <div className="p-3.5 rounded-xl bg-emerald-950/30 border border-emerald-500/30 flex items-center gap-3">
                ...
                <div className="text-xs text-slate-300">
                  Signing in as <strong className="text-white">{selectedUserFor2FA?.name}</strong> (
                  {selectedUserFor2FA?.role})
                </div>
              </div>
              ...
              <form onSubmit={handleVerify2FA} className="space-y-4">
                {twoFactorMethod === 'totp' && (
                  <div>
                    <label className="block text-xs font-semibold text-slate-300 mb-1.5">
                      Enter 6-digit TOTP code from Google Authenticator / Authy:
                    </label>
                    <Input
                      type="text"
                      maxLength={6}
                      ...
                      className="text-center font-mono tracking-widest text-lg bg-slate-900 border-slate-700 text-white"
                      autoFocus
                      required
                    />
                    <div className="mt-1.5 text-[11px] text-slate-500 flex items-center justify-between">
                      <span>Demo Quick Passcode: <strong className="text-emerald-400">123456</strong></span>
                      <span className="text-emerald-400">Time sync active</span>
                    </div>
                  </div>
                )}
                ...
                {twoFactorMethod === 'sms' && (
                  <div>
                    <label className="block text-xs font-semibold text-slate-300 mb-1.5">
                      SMS security code sent to +1 •••• •••• {twoFactorConfig.phoneLast4}:
                    </label>
                    <Input
                      type="text"
                      maxLength={6}
                    ...
                    <div className="mt-2 flex items-center justify-between text-[11px] text-slate-400">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          toast('New SMS verification code dispatched.', 'info');
                          setTwoFactorCode('774829');
                        }}
                ...
                {twoFactorMethod === 'backup' && (
                  <div>
                    ...
                    <Input
                      type="text"
                      placeholder="e.g. A7B2-C9F1"
                      value={twoFactorCode}
                      onChange={(e) => setTwoFactorCode(e.target.value.toUpperCase())}
                      className="text-center font-mono tracking-widest uppercase bg-slate-900 border-slate-700 text-white"
                      autoFocus
                      required
                    />
                    <p className="mt-1 text-[11px] text-slate-500">
                      Tip: Use one of your saved backup codes: <code className="text-emerald-400">A7B2-C9F1</code>
                    </p>
                  </div>
                )}
                ...
                <button
                  type="button"
                  onClick={() => setIs2FAPrompt(false)}
                  className="w-full text-center text-xs text-slate-400 hover:text-slate-200 flex items-center justify-center gap-1.5 pt-1"
                >
            ...
            /* ------------------------------------------------------------- */
            /* STEP 1: PRIMARY CREDENTIALS / DEMO PERSONA SELECTION          */
            /* ------------------------------------------------------------- */
            <>
                  ...
                  {allUsers.map((u) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] bg-slate-700 text-slate-300 px-1.5 py-0.5 rounded font-mono">2FA Active</span>
              ...
              <div className="grid grid-cols-2 gap-3 pt-1">
                <button
                  type="button"
                  onClick={() => handleQuickDemoSelect(allUsers[0])}
                ...
                <button
                  type="button"
                  onClick={() => handleQuickDemoSelect(allUsers[1])}
                  className="flex items-center justify-center gap-2 py-2 px-3 rounded-lg border border-slate-700 bg-slate-900 text-slate-300 hover:bg-slate-800 text-xs font-semibold transition-colors"
                >
